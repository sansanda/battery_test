...
import visa
from visa import six
from visa.six.moves.urllib.parse import parse_qsl
...
def _test_or_live_environment():
    if visa.api_key is None:
        return
    match = re.match(r"sk_(live|test)_", visa.api_key)
    if match is None:
        return
...
def dashboard_link(request_id):
    return "https://dashboard.visa.com/{env}/logs/{reqid}".format(
